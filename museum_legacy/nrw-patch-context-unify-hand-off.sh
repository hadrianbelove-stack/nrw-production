...
### Deprecated (Legacy)
# LEGACY_NRW_SYNC_REMOVED - **NRW_SYNC + manifest + sha** handoffs are **retired**. Existing archives move to `bundle_history/` (git-ignored) for audit only.
...
# LEGACY_NRW_SYNC_REMOVED # Insert/replace a Handoff section: remove any legacy "NRW_SYNC handoff" text block and add new policy near the top after ground rules.
awk -v RS= -v ORS="\n\n" -v insert="$INSERT" '
{
  text=$0
# LEGACY_NRW_SYNC_REMOVED   # Drop any prior "Handoff"/"Session Handoff" sections mentioning NRW_SYNC
# LEGACY_NRW_SYNC_REMOVED   gsub(/##[[:space:]]*(Session Handoff|Handoff)[^\n]*\n([^#]|#[^#])*?(NRW_SYNC[^#]*\n([^#]|#[^#])*)*/,"",text)
...
# LEGACY_NRW_SYNC_REMOVED echo "- ${STAMP}: Context updated — unified Working Set handoff; legacy NRW_SYNC deprecated" >> PROJECT_LOG.md
