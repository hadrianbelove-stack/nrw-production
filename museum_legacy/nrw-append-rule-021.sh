#!/bin/bash
# nrw-append-rule-021.sh — VISIBILITY GUARANTEE
set -euo pipefail
...
# LEGACY_NRW_SYNC_REMOVED - Every working session must provide a code snapshot to the assistant (or the latest NRW_SYNC bundle).
# LEGACY_NRW_SYNC_REMOVED - Assistants must base all recommendations on the most recent provided code snapshot or unpacked NRW_SYNC.
