#!/usr/bin/env python3
"""Test Wikipedia scraper standalone with 10 movie titles."""

import json
from wikipedia_scraper_playwright import WikipediaScraperPlaywright

def test_wikipedia_scraper():
    """Test Wikipedia scraper with 10 movie titles."""
    ...
    print(f"🧪 Testing Wikipedia scraper with {len(test_movies)} movies...")
    print("=" * 60)
    ...
    for i, movie in enumerate(test_movies, 1):
        ...
        try:
            # Test the scraper
            result = scraper.find_wikipedia_url(title, year)

            if result:
                print(f"✅ SUCCESS: Found Wikipedia URL")
                print(f"   URL: {result}")
                successes += 1
                results.append({"title": title, "year": year, "url": result, "status": "success"})
            else:
                print(f"❌ FAILED: No Wikipedia URL found")
                failures += 1
                results.append({"title": title, "year": year, "url": None, "status": "failed"})

        except Exception as e:
            print(f"💥 ERROR: {str(e)}")
            failures += 1
            results.append({"title": title, "year": year, "url": None, "status": "error", "error": str(e)})
    ...
    # Summary
    print("\n" + "=" * 60)
    print("🏁 WIKIPEDIA SCRAPER TEST SUMMARY")
    print("=" * 60)
