...
class MovieEnricher:
    ...
    def _load_and_validate_data(self):
        ...
        Returns:
            tuple: (display_data, existing_movies, movie_lookup) or None on failure.
        """
        ...
        self.ctx.logger.info(f"Enrichment loaded data.json: {len(existing_movies)} movies")

        return (display_data, existing_movies, movie_lookup)

    def _build_enrichment_queue(self, target_id, existing_movies, movie_lookup):
        """Build the list of movie IDs to enrich.

        Handles single-movie mode, newly_available.json loading, pre-order link
        finding, catch-up retries, and batch limiting.

        Returns:
            tuple: (movie_ids_to_enrich, tracking_data, tracking_changed) or None
                   if nothing to enrich.
        """
        ...
        if target_id:
            ...
            return (movie_ids_to_enrich, tracking_data, tracking_changed)
    ...
    def _setup_loop_environment(self, tracking_data):
        """Preload IMDb dataset, validate tracking data, set up signal handlers
        and timeout constants.

        Returns:
            tuple: (per_movie_timeout, loop_timeout, timeout_handler) or None on failure.
        """
    ...
    def _enrich_single_movie(self, movie_id, existing_movies, movie_lookup, tracking_data, per_movie_timeout):
        ...
        Returns:
            dict with keys:
                'status': 'enriched' | 'failed' | 'reverted' | 'timeout' | 'error' | 'skipped' | 'not_in_data' | 'not_in_tracking'
                'deferred_entry': dict or None
        ...
        signal.alarm(per_movie_timeout)
        try:
            ...
            print(f"  \u23f0 {movie_data.get('title', movie_id)} timed out after {per_movie_timeout}s at step '{_step}' \u2014 will retry next run")
            ...
            self._error_details.append({
                ...
                'error_message': f'Exceeded {per_movie_timeout}s at step {_step}',
            })
    ...
    def _sync_tracking_and_revert(self, movie_ids, existing_movies, movie_lookup, tracking_data, tracking_changed):
        ...
        Returns:
            tuple: (deferred_entries_list, reverted_count)
        """
        deferred_entries = []
        _enrich_reverted = 0
        ...
        return (deferred_entries, _enrich_reverted)

    def _save_and_post_process(self, display_data, existing_movies, movie_ids, movie_lookup, enriched_count, initial_movie_count):
        """Categorize, save data.json, host trailers, and purge reverted movies.
        ...
        if not self.host._safe_save_data_json(display_data, existing_movies, label="enrichment"):
            return False
        print(f"\u2705 Enriched {enriched_count}/{len(movie_ids)} movies ({len(existing_movies)} total, was {initial_movie_count} at load)")
    ...
    def _write_enrichment_metrics(self, movie_ids, enriched_count, deferred_details, start_time):
        """Write enrichment run metrics to metrics/enrichment_run.json."""
        try:
            enrichment_metrics = {
                "timestamp": datetime.now().isoformat(),
                "movies_requested": len(movie_ids),
                "movies_enriched": enriched_count,
                "movies_deferred": len(movie_ids) - enriched_count,
    ...
    def enrich_newly_available_movies(self, target_id=None) -> int:
        ...
        # Phase A: Load and validate data.json
        loaded = self._load_and_validate_data()
        if not loaded:
            return 0
        display_data, existing_movies, movie_lookup = loaded
        _initial_movie_count = len(existing_movies)

        # Phase B: Build enrichment queue
        queue = self._build_enrichment_queue(target_id, existing_movies, movie_lookup)
        if not queue:
            self.host.purge_removed_movies()
            return 0
        movie_ids_to_enrich, tracking_data, tracking_changed = queue

        # Phase C: Setup loop environment
        setup = self._setup_loop_environment(tracking_data)
        if not setup:
            return 0
        per_movie_timeout, loop_timeout, _timeout_handler = setup

        # Phase D: Main enrichment loop
        enriched_count = 0
        deferred_details = []
        ...
        for movie_id in movie_ids_to_enrich:
            ...
            if result['status'] == 'enriched':
                enriched_count += 1
            if result['deferred_entry']:
                deferred_details.append(result['deferred_entry'])
        ...
        # Phase F: Save + post-process
        self._save_and_post_process(
            display_data, existing_movies, movie_ids_to_enrich,
            movie_lookup, enriched_count, _initial_movie_count
        )

        # Phase G: Write metrics
        self._write_enrichment_metrics(
            movie_ids_to_enrich, enriched_count, deferred_details, _enrich_start
        )

        # Final in-memory purge
