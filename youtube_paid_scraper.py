...
class YouTubePaidScraper:
    ...
    def __init__(self, cache_file: str = "cache/youtube_cache.json", config: dict = None, headless: bool = True):
        ...
        if config:
            ...
            try:
                ...
                self.config = get_scraper_config(full_config, "youtube_scraper")
            except Exception:
    ...
    def _scrape_youtube_movies_page(self, url: str) -> List[Dict]:
        ...
        print(f"Scraping YouTube page: {url}")
        ...
        try:
            ...
            for selector in selectors:
                ...
                if elements:
                    ...
                    print(f"Found {len(elements)} potential films using selector: {selector}")
                    break

            if not film_elements:
                # Fallback: look for any elements with video links
                potential_films = page.locator('a[href*="/watch"]').all()
                if potential_films:
            ...
            if self.config.get("screenshots_enabled", True):
                try:
                    ...
                    page.screenshot(path=f"screenshots/youtube_error_{int(time.time())}.png")
                except:
                    pass
            raise e
    ...
    def discover_films(self, max_pages: int = 5, search_queries: List[str] = None) -> List[Dict]:
        ...
        print(f"Starting YouTube discovery (max_pages: {max_pages})")
