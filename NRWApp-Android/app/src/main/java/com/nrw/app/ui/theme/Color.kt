package com.nrw.app.ui.theme

import androidx.compose.ui.graphics.Color

// NRW Brand Colors - matching website design
val Background = Color(0xFF0A0A0A)
val BackgroundSecondary = Color(0xFF1A1A2E)  // Dark blue-purple from website
val BackgroundGradientEnd = Color(0xFF1A1A2E)
val Primary = Color(0xFF00D4AA)  // Teal accent from website
val StaffPickRed = Color(0xFFDC143C)  // Crimson for staff picks
val RestorationGold = Color(0xFFC8A951)  // Antique gold for restorations
val ScreeningGold = Color(0xFFFFD700)    // Gold for virtual screenings
val TextPrimary = Color(0xFFFFFFFF)
val TextSecondary = Color(0xFFBBBBBB)
val TextMuted = Color(0xFF888888)
val Green = Color(0xFF34C759)  // Apple system green (RT Fresh)
val Red = Color(0xFFFF3B30)    // Apple system red (RT Rotten)

// Service colors for watch buttons
// Source of truth: assets/service-colors.json
val TrailerRed = Color(0xFFE50914)   // YouTube/Trailer button
val AmazonOrange = Color(0xFFFF9900)
val NetflixRed = Color(0xFFE50914)
val TubiOrangeRed = Color(0xFFFA382F)
val HuluGreen = Color(0xFF1CE783)
val MaxPurple = Color(0xFFB537F2)  // Fixed: was 0xFF741DDA, synced with CSS
val DisneyBlue = Color(0xFF113CCF)
val PeacockBlack = Color(0xFF000000)
val ParamountBlue = Color(0xFF0064FF)
val MubiRed = Color(0xFFDA2128)
val CriterionBlack = Color(0xFF000000)
val FandangoOrange = Color(0xFFFF7300)
val AmcGreen = Color(0xFF1BB74B)
val PlexGold = Color(0xFFE5A00D)
val ShudderRed = Color(0xFF8B0000)
